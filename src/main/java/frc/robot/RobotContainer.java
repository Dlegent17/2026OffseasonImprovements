...
public class RobotContainer {
   ...
   public final FloorSubsystem floor = new FloorSubsystem();
    ...
    private final VisionSwerveSystem visionSwerveSystem = new VisionSwerveSystem(drivebase.getPoseEstimator(), turret, drivebase);
...
private final Command fixedTriggerShootRoutine =
    Commands.parallel(
     Commands.run(() -> shooter.setDynamicShooter(visionSwerveSystem.getDistanceToHubMeters()), shooter),
        new SnapToTagCommand(drivebase, visionSwerveSystem).withTimeout(0.75), indexer.feedToShooterCommand(), floor.fixedIntake()
    );
...
private final Command fixedStopShootRoutine = Commands.parallel(
        ...
        Commands.runOnce(() -> floor.stopIntake(), floor),
        Commands.runOnce(() -> shooter.stopShooterAndStartHoming(), shooter), 
    ...
    Commands.sequence(intake.getPivotDown());
    ...
    private void configureBindings() {
      ...
      // Default drive command
        ...
        driverXbox.rightBumper().whileTrue(Commands.run(()-> shooter.hoodUp()));
         driverXbox.leftBumper().whileTrue(Commands.run(()-> shooter.hoodDown()));
         driverXbox.b().onTrue(stowIntakeCommand);
         driverXbox.a().onTrue(passingTriggerShootRoutine);
        driverXbox.povUp().whileTrue(Commands.run(() -> floor.fixedIntake()));
        driverXbox.povDown().whileTrue(Commands.run(() -> floor.reverseIntake()));
    }
